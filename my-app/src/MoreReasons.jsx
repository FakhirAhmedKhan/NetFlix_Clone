    ...
    <section className="w-full max-w-6xl mx-auto px-6 py-16 ">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 ">
        {reasons.map((item, index) => (
          <div
            key={index}
            className="bg-[#141414] rounded-xl  p-6 shadow-lg transition hover:scale-[1.02] hover:bg-[#1f1f1f] bg-gradient-to-r from-[rgb(30,22,41)] via-[rgb(28,26,53)] to-[rgb(30,22,41)] p-10 "
          >
            <h3 className="text-xl font-semibold text-white mb-2">
              {item.title}
            </h3>
            <p className="text-gray-100 leading-relaxed text-[17px] mt-8">
              {item.description}
            </p>
            <div className="flex justify-end items-end">{item.icon}</div>
          </div>
        ))}
      </div>
    </section>
